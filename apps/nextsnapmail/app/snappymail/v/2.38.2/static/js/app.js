	...
	*/
	...
	class EmailAddressesComponent {
					...
					if (datalist.inputValue !== value) {
						...
						value.length && self.options.autoCompleteSource(
							...
							items => {
								...
								items?.forEach(item => {
									const option = 'string' === typeof item
										? new Option(item)
										: new Option(item.label || item.value, item.label || item.value);
									if ('string' !== typeof item && item.value) {
										option.dataset.value = item.value;
									}
									datalist.append(option);
									chars = Math.max(chars, (item.label || item.value || item).length);
								});
		...
		_parseInput(force) {
			...
			const selectedOption = [...datalist.options].find(option => option.value === val);
			if (selectedOption?.dataset.value) {
				val = selectedOption.dataset.value;
			}
					...
					if (!iError && isArray(data.Result)) {
						fResponse(
							data.Result.map(item => {
								...
								const value = (new EmailModel(item[0], item[1])).toLine();
								return item[2]?.label ? {value, label: item[2].label} : value;
							})
							.filter(v => v)
						);
