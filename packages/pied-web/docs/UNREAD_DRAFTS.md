...
minute while visible. Since 1.9.1, a successful background send invalidates an earlier
compose-close refresh, so a just-sent edited Draft cannot be redrawn from that stale response;
the global Feed refreshes on the same signal. Refreshing reloads the first ten matching drafts. Switching accounts
